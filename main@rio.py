...
def dice(ws, ls):
    ...
    if obj["Betset"][urut]["Mode1"]["Toggle"] == "ON" or obj["Betset"][urut]["Mode1"]["Toggle"] == "on" or obj["Betset"][urut]["Mode1"]["Toggle"] == "On":
        ...
        payin = int(float(obj["Betset"][urut]
                          ["Fibonacci1"]["Prebet"])*(10 ** 8))
        fibo = False
        fbtmp = 1
        fbcount = 1
        ...
        while True:
            ...
            if obj["Betset"][urut]["Mode1"]["Toggle"] == "ON" or obj["Betset"][urut]["Mode1"]["Toggle"] == "on" or obj["Betset"][urut]["Mode1"]["Toggle"] == "On":
                ...
                if jsn["PayOut"] != 0:
                    ...
                    # BET SETTING
                    if tmpprofit < profitpay:
                        if no_win >= startfish and no_win <= stopfish:
                            if tmplosefish > 0:
            ...
            if obj["Auto Wd"]["Toggle"] == "On" or obj["Auto Wd"]["Toggle"] == "ON" or obj["Auto Wd"]["Toggle"] == "on":
                if float(rev(str(bal))) > float(obj["Auto Wd"]["If Balance"]):
                    ...
                    with open("history.log", "a+") as f:
                        f.write("Withdraw " +
                                str(rev(str(withdraw["Pending"])))+"\n")
                    sys.exit()
    except:
        try:
            with open("history.log", "a+") as f:
                f.write("["+datetime.now().strftime('%Y/%m/%d %H:%M:%S')+"] Win Streak "+str(total_win)+" Lose Streak "+str(
                    total_lose)+" | Balance "+str(float(int(jsn["StartingBalance"]) + int(jum))/(10 ** 8))+" Profit "+str(prof)+"\n")
        except:
...
try:
    ...
    print(profitcolor + putih2 + " Balance ", res + str((float(int(currbalance))/(10 ** 8))) +
